...
alter table historial_venta add id_vehiculo int;
alter table historial_venta add foreign key(id_vehiculo) references inventario(id);
...
alter table historial_servicios add id_proveedor int;
alter table historial_servicios  add foreign key(id_proveedor) references proveedores(id_p);

s

alter table historial_venta add fecha date;


create type tipo_servicio as enum('reparacion', 'mantenimiento');

alter table historial_servicios add type tipo_servicio not null;
alter table historial_venta drop column fecha;
...
alter table departamento_ventas add fecha date ;
...
select * from historial_servicios hs;
...
update historial_venta set id_vehiculo = 1 where id_venta = 1;
update historial_venta set id_vehiculo = 2 where id_venta = 2;
update historial_venta set id_vehiculo = 3 where id_venta = 3;
update historial_venta set id_vehiculo = 4 where id_venta = 4;

select * from historial_servicios hs ;

update historial_servicios  set id_proveedor = 1 where type = 'mantenimiento' and codigo = 1;
update historial_servicios  set id_proveedor = 2 where type = 'mantenimiento' and codigo = 2;
update historial_servicios  set id_proveedor = 3 where type = 'reparacion' and codigo = 2;

update historial_venta set fecha = '2020-14-05' where id_empleado 
...
select * from departamento_ventas dv ;
...
select i.id, hs.type, hs.id_empleado  from inventario i inner join historial_servicios hs on i.id = hs.id_vehiculo ;

-- CONSULTA 4

select p.id_p, hs.type from proveedores p inner join historial_servicios hs on p.id_p = hs.id_proveedor where type = 'mantenimiento';

-- CONSULTA 5

select e.id_empleado, sum(dv.comisiones) from departamento_ventas dv inner join empleados e on e.id_empleado = dv.id_empleado where 
